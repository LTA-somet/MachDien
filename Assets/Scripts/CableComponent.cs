...
public class CableComponent : MonoBehaviour
{
	...
	[SerializeField] private float maxCollisionDistance = 0.1f;
    ...
    void SolveCollisionConstraint()
    {
        ...
        for (int pointIdx = 0; pointIdx < segments + 1; pointIdx++)
        {
            Vector3 particlePosition = points[pointIdx].Position;

            // Perform a raycast from the particle position to check for collisions
            if (Physics.Raycast(particlePosition, Vector3.down, out hit, maxCollisionDistance))
            {
                // If collision occurs, adjust particle position to avoid penetration
                Vector3 newPosition = hit.point + hit.normal * collisionOffset;
                points[pointIdx].Position = Vector3.Lerp(points[pointIdx].Position, newPosition, stiffness);
            }
        }
    }
    #endregion
